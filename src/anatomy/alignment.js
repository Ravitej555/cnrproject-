...
export function detailedSkeletonTransform(bounds, { mirrorX = true } = {}) {
  ...
  const rawScale = DETAILED_BODY_REGISTRATION.runtime.height / (complete.max.y - complete.min.y);
  const scale = rawScale * 0.984;
  return {
    scale,
    position: {
      x: -boundsCenter(complete, 'x') * scale,
      y: DETAILED_BODY_REGISTRATION.runtime.floorY - complete.min.y * scale + 0.016,
      z: depthOffset(boundsCenter(complete, 'z') * scale) + 0.018
    },
    bounds: complete
  };
}
