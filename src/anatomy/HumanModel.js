...
export const ARM_VESSEL_CONTROL_POINTS = Object.freeze([
  Object.freeze([.08, 1.55, .15]),
  Object.freeze([.26, 2.05, .06]),
  Object.freeze([.48, 1.75, .00]),
  Object.freeze([.65, 1.15, -.06]),
  Object.freeze([.74, 0.50, -.07]),
  Object.freeze([.79, 0.20, -.08]),
]);
...
export class HumanModel {
  constructor() {
    ...
    this.skinMaterial = new THREE.MeshPhysicalMaterial({
      ...
      roughness: 0.52,
      metalness: 0.0,
      transparent: DEFAULT_SKIN_OPACITY < 0.98,
      opacity: DEFAULT_SKIN_OPACITY,
      depthWrite: true,
      depthTest: true,
      ...
      thickness: 0,
      sheen: 0.35,
      sheenColor: new THREE.Color(0xffc2b2),
      sheenRoughness: 0.40,
      clearcoat: 0.08,
    ...
    this.highlightMaterial = new THREE.MeshPhysicalMaterial({
      ...
      roughness: 0.35,
      metalness: 0.05,
      emissive: new THREE.Color('#c89a66'),
  ...
  async loadDetailedSkeleton(onProgress) {
    ...
    asset.pickables.forEach(mesh=>{this.pickables.push(mesh);this.structureMeshes.set(mesh.userData.structureId,mesh);mesh.userData.baseMaterial=mesh.material;this.captureMaterialState(mesh.material);});
  ...
  async loadScannedSkin(onProgress,loadAsset=loadDracoAnatomyAsset) {
    ...
    this.skinMeshes = [];
    const isFullSkin = (this.layerOpacity.skin ?? DEFAULT_SKIN_OPACITY) >= 0.95;
    entries.forEach(({node,fmaId})=>{
      if (node.geometry) {
      ...
      node.renderOrder = isFullSkin ? 0 : 5;
      node.userData = { ...node.userData, structureId: 'skin', fmaId, baseOpacity: DEFAULT_SKIN_OPACITY, baseMaterial: this.skinMaterial };
      this.skinMeshes.push(node);
    });

    sourceMaterials.forEach(mat=>mat.dispose());
    this.layers.skin.add(asset.root);
    this.fallbackSkin.visible = false;
    this.skinAsset = asset.root;

    const skinOpacity = this.layerOpacity.skin ?? DEFAULT_SKIN_OPACITY;
    this.setOpacity('skin', skinOpacity);
  ...
  buildVessels() {
    ...
    const paths=[
      [V(.06,1.6,.22),V(.05,.6,.16),V(0,-.15,.10)],
      [V(-.10,-.15,.10),V(-.28,-1.5,.14),V(-.26,-2.85,.10)],
      [V(.10,-.15,.10),V(.28,-1.5,.14),V(.26,-2.85,.10)]
    ];
    ...
    [-1,1].forEach((sign,i)=>g.add(new THREE.Mesh(new THREE.TubeGeometry(createArmVesselCurve(sign),40,.016,7,false),i?artery:vein)));
    const headCurve=new THREE.CatmullRomCurve3([V(.06,1.7,.12),V(0,2.35,.14),V(0,2.75,.10)]);
    g.add(new THREE.Mesh(new THREE.TubeGeometry(headCurve,32,.022,7,false),vein));
    this.pulseMaterials=[artery,vein];
  }

  alignVesselsToDetailedAnatomy() {
    this.layers.circulatory.position.z=0.12;
  }
  ...
  setOpacity(key, opacity){
    ...
    if (key === 'skin' && this.skinMaterial) {
      const isFullSkin = opacity >= 0.95;
      this.skinMaterial.opacity = isFullSkin ? 1.0 : opacity;
      this.skinMaterial.transparent = !isFullSkin;
      this.skinMaterial.depthWrite = true;
      this.skinMaterial.depthTest = true;
      this.skinMaterial.needsUpdate = true;

      this.skinMeshes.forEach(mesh => {
        mesh.renderOrder = isFullSkin ? 0 : 5;
      });

      // When skin is in full opaque mode, occlude internal layers cleanly so zero geometry can poke through
      if (isFullSkin) {
        if (this.layers.skeleton) this.layers.skeleton.visible = false;
        if (this.layers.circulatory) this.layers.circulatory.visible = false;
        if (this.layers.organs) this.layers.organs.visible = false;
      } else {
        if (this.layers.skeleton) this.layers.skeleton.visible = (this.layerOpacity.skeleton ?? 1) > 0;
        if (this.layers.circulatory) this.layers.circulatory.visible = (this.layerOpacity.circulatory ?? 1) > 0;
        if (this.layers.organs) this.layers.organs.visible = (this.layerOpacity.organs ?? 1) > 0;
      }
    }
    ...
    normal.forEach(mat=>{const state=this.captureMaterialState(mat);let base=state.opacity;if(key==='skeleton'&&base<0.5){base=1;state.opacity=1;}mat.transparent=(key==='skin')?(opacity<0.95):(opacity<.995||base<.995);mat.opacity=key==='skin'?opacity:Math.min(base,opacity);mat.depthWrite=key==='skin'?(opacity>0.6):opacity>.75&&base>.75;if(key==='skin'){mat.transmission=0;mat.thickness=0;mat.side=THREE.FrontSide;}});
  ...
  restoreAll(){
    Object.entries(this.layers).forEach(([key,layer])=>{layer.visible=this.layerOpacity[key]>0;});
    this.root.traverse(node=>{
      if(!node.isMesh)return;
      node.visible=true;
      if(node.userData.actionMaterial){
        node.material.dispose();
        node.material=node.userData.actionMaterial;
        delete node.userData.actionMaterial;
      }
    });
  ...
  updateHighlights(){
    ...
    materialOrganIds.forEach((ids, mat)=>{
      ...
      if(mat.emissive&&base?.emissive){
        ...
        if(selected) mat.emissive.setHex(0xc89a66);
        else if(hovered) mat.emissive.setHex(0x00b0ff);
        else mat.emissive.add(base.pulseEmissive||new THREE.Color(0));
      }
      mat.emissiveIntensity = selected ? 0.85 : (hovered ? 0.85 : (base?.pulseIntensity || (base?.emissiveIntensity ?? 1)));
    });
  }
